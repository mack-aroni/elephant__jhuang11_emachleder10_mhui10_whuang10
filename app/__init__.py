...
def story_edit():
    ...
    for x in c4.execute("SELECT contributors FROM stories"):
        ...
        count = count + 1
    return render_template('list_edit.html', title_list = list_titles)
db.close()
...
def story_editor():
    ...
    entire = c5.execute("SELECT entire FROM stories WHERE title = ?", (title,))
    entire = c5.fetchone()[0]
    new_entire = entire + "\n" + new_text
    c5.execute("UPDATE stories SET entire = ? WHERE title = ?", (new_entire, title))
    
    contributors = c5.execute("SELECT contributors FROM stories WHERE title = ?", (title,))
    contributors = c5.fetchone()[0]
    new_contributors =  contributors + username + ","
    c5.execute("UPDATE stories SET contributors = ? WHERE title = ?", (new_contributors, title))
    
    contributions = c5.execute("SELECT contributions FROM users WHERE username = ?", (username,))
    contributions = c5.fetchone()[0]
